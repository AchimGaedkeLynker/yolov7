...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix='', kpt_label=False):
        # num_kpts = 17 # for person keypoints
        num_kpts = 3 # for copepod_large
            ...
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        ...
                        if kpt_label:
                            assert l.shape[1] == 5 + 3 * num_kpts, f'labels require {5 + 3 * num_kpts} columns each'
                            ...
                            kpts = np.zeros((l.shape[0], 5 + 2 * num_kpts))
                            for i in range(len(l)):
                                kpt = np.delete(l[i,5:], np.arange(2, l.shape[1]-5, 3))  #remove the occlusion parameter from the GT
                                kpts[i] = np.hstack((l[i, :5], kpt))
                            l = kpts
                            assert l.shape[1] == 5 + 2 * num_kpts, f'labels require {5 + 2 * num_kpts} columns each after removing occlusion parameter'
                        else:
                        ...
                        ne += 1  # label empty
                        l = np.zeros((0, 5 + 2 * num_kpts), dtype=np.float32) if kpt_label else np.zeros((0, 5), dtype=np.float32)
    ...
    if n:
        ...
        if use_segments:  # warp segments
            ...
            if kpt_label:
                ...
                xy_kpts[:, :2] = targets[:,5:].reshape(n*num_kpts, 2)  #num_kpt is hardcoded to 17
                xy_kpts = xy_kpts @ M.T # transform
                xy_kpts = (xy_kpts[:, :2] / xy_kpts[:, 2:3] if perspective else xy_kpts[:, :2]).reshape(n, 2*num_kpts)  # perspective rescale or affine
                ...
                xy_kpts[:, list(range(0, 2*num_kpts, 2))] = x_kpts
                xy_kpts[:, list(range(1, 2*num_kpts, 2))] = y_kpts
